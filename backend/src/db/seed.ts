import { initializeDatabase, run, saveDatabase } from "./schema.js";
...
async function seed() {
  ...
  auditEntries.forEach((entry, i) => {
    ...
    run(
      `INSERT INTO audit_entries (id, timestamp, actor, action, resource, risk_level, compliance_relevant) VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [randomUUID(), timestamp, entry.actor, entry.action, entry.resource, entry.risk_level, entry.compliance_relevant]
    );
  });
  ...
  riskAssessments.forEach((assessment) => {
    run(
      `INSERT INTO risk_assessments (id, entity_id, entity_type, overall_score, risk_level, factors, recommendations, assessed_by, assessed_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [randomUUID(), assessment.entity_id, assessment.entity_type, assessment.overall_score, assessment.risk_level, assessment.factors, assessment.recommendations, assessment.assessed_by, new Date().toISOString()]
    );
  });
  ...
  complianceChecks.forEach((check) => {
    ...
    run(
      `INSERT INTO compliance_checks (id, target, target_type, frameworks, status, score, results, checked_by, started_at, completed_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [randomUUID(), check.target, check.target_type, check.frameworks, check.status, check.score, check.results, check.checked_by, startedAt, now.toISOString()]
    );
  });
